...
import { buildAudioConvertBatCommand } from '@/utils/BuildAudioConvertBatCommand'

/**
 * 根据用户配置生成 Windows 批处理脚本（.bat）
 *
 * @param form 用户配置的音频格式转换表单
 * @returns 完整的 .bat 文件内容字符串（使用 \r\n 换行）
 */
export function buildAudioConvertBatScript(form: AudioConvertForm): string {
    // 生成单条 ffmpeg 命令片段
    const cmd = buildAudioConvertBatCommand(form)

    // 把参数转成中文显示
    const targetText = form.targetFormat.toUpperCase()
    ...
    const bitrateText = isLosslessTarget ? '不适用（无损）' : form.audioBitrate
...
echo.
echo ============================================
echo   FFmpeg 批量音频格式转换
echo ============================================
echo   输入格式=${form.inputPattern}
echo   目标格式=${targetText}
echo   音频编码=${audioCodecText}
echo   音频码率=${bitrateText}
echo   跳过已存在=${skipText}
echo   输出目录=%OUT%
echo ============================================
echo.

REM 遍历当前文件夹的所有 ${form.inputPattern}
